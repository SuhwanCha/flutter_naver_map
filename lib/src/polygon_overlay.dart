...
/// - 테두리 두께
/// - 테두리 색상
class PolygonOverlay extends Equatable {
  /// ### 폴리곤 오버레이 생성
  ...
  Map<String, dynamic> _toJson() {
    assert(
      coordinates.length >= 3,
      'coordinates.length must be greater than 3',
    );
    ...
    addIfPresent('holes', holes?.map(serializeLatLngList).toList());
    return json;
  }
...
Map<String, PolygonOverlay> _keyByPolygonId(Iterable<PolygonOverlay> polygons) {
  ...
  return Map<String, PolygonOverlay>.fromEntries(
    polygons.map(
